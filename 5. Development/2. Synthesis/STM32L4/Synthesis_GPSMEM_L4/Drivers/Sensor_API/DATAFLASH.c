...
/*
 * @Description: Function To initialise peripherals that interface with flash chips.
 * Configures the SPI settings, GPIO Settings and checks to see how many flash chips
 * are online. The function initialises SPI on SPI2 and attempts to read the status
 * register of each chip. The status of each chip is stored in an 8-byte variable.
 * Each chip is represented by a bit in bit position 0 - 3. If more flash chips are
 * desired, the additional bits of the byte can be used.
 *
 * @param: chipstatus - pointer to an 8 bit variable that stores the status of each chip
 *
...
/*
 * Function to configure the flash chip page size
 *
 */
HAL_StatusTypeDef Config_Flash_Chip(uint8_t chipnumber, Chip_Status_t status)
{
	FLASH_CONFIG_PageSize(chipnumber, 256);
	//GET STATUS REGISTER TO CHECK THAT PROGRAMMING WAS SUCCESSFUL
	uint8_t* Data_p = 0x00;
	Data_p = FLASH_GetStatusRegister(chipnumber);
	if((*Data_p& 0b1) != 1)
	{
		return HAL_ERROR;
	}
...
/*
 * @Description function used to verrify chips are working by reading the ID
 * A Valid ID  consists of 5 bytes with fixed values which are as follows:
 * Byte 0 = 0x1F
 * Byte 1 = 0x28
 * Byte 2 = 0x00
 * Byte 3 = 0x01
...
/**
  * @DESCRIPTION Checks to see if the current flash chip is available for data storage
  * 		     An available chip is not at full capacity and has enough space to store
  * 		     the desired amount of data. The function performs a check to see if the
