...
uint32_t RTC_To_Unix_Time(RTC_TimeTypeDef* rtctime, RTC_DateTypeDef* rtcdate)
{
		time_t t;
		struct tm *timepointer;
		t = time(NULL);
		...
		timepointer->tm_year = rtcdate->RTC_Year+2000-1900;
		timepointer->tm_mon = rtcdate->RTC_Month -1;
		timepointer->tm_mday = rtcdate->RTC_Date;
		timepointer->tm_hour = rtctime->RTC_Hours ;
		timepointer->tm_min = rtctime->RTC_Minutes;
		timepointer->tm_sec = rtctime->RTC_Seconds;
		time_t res = mktime(timepointer);
		return (uint32_t) res;
}
void get_Time(uint32_t Eptime,char* timeString)
{
	time_t current_time = (time_t)Eptime;
	struct tm *timeinfo = localtime(&current_time);
	strftime(timeString, 60, "Log Time: %Y-%m-%d %H:%M:%S\r\n", timeinfo);
}
//==========================================================================
