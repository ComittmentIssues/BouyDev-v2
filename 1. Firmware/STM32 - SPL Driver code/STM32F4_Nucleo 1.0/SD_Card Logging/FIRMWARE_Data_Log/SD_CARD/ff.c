...
)
{
	...
	for (;;) {
		...
		w = f_convert(w, 1);			/* Convert ANSI/OEM to Unicode */
...
#ifdef _EXCVT
			w = f_convert(w, 0);		/* Unicode -> OEM code */
...
#if _USE_LFN
			w = f_convert(ff_wtoupper(f_convert(w, 1)), 0);
#else
...
WCHAR f_convert (WCHAR	chr,UINT dir)
{
	WCHAR c;
