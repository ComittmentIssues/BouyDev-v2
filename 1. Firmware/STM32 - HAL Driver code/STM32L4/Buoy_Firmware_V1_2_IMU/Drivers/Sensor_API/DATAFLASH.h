...
typedef enum
{
	Active = 1,	//Chip is designated this status if it available, is going to be used immediately to store data
	Next_Active = 2, //Chip is online, available and will be used for storage when Active Chip is full
	Inactive = 3,	 //Chip is online, available but not used.
	Full = 4,		 //Chip storage has reached capacity and can no longer be used
	Back_UP = 5,	 //Chip is designated as back up chip. Will not be used if there is a chip with a higher priority
	Erased = 0	 //Chip has just completed an Erased Option and is completely empty.
}Chip_Status_t;
...
uint8_t FLASH_Is_Available(uint8_t chipnumber,int datasize);//Checks to see if chip has memory available
//READ
